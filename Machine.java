...
public class Machine {
    ...
    public static void withdrawMoney(User user) {
        ...
        if ((((withdraw % 2000) % 500) % 200) % 100 != 0) {
            System.out.println("Amount " + withdraw + " cannot be dispatched");
            return;
        }
        if (user.bankBalance >= withdraw) {
            user.bankBalance -= withdraw;
            String save = String.format("Successfully withdrawn Rs.%d\n", withdraw);
            while (withdraw != 0) {
    ...
    public static void deposit(User user) {
        ...
        if (sum == n) {
            ...
            String save = String.format("Successfully deposied Rs.%d\n", n);
            user.transactions += save;
        } else {
            System.out.println("Incorrect Denominations");
            deposit(user);
        }
    }
}
